...
/// FATAL must be the last level, otherwise first line of AppendLog should be modified
/// This is compatible with libevent log level definition
...
/// use __PRETTY_FUNCTION__(gcc) or __FUNCSIG__ (vs) instead will be more powerful
#define LogAppend(level, ...) Logger::GetInstance()->AppendLog(Utilis::level, \
...
class Logger : Utilis::NonCopyable {
 public:
  ...
  /// It will auto log the file name, func name, line number, time happened and line break
