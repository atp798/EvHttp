...
void Logger::AppendLog(LogLevel level, const char* file, const char* func, int line, const char* fmt, ...){
  if (level < level_ || level >LL_FATAL) { return; }
  ...
  int len = strlen(msg);
  snprintf(msg + len, sizeof(msg) - len -1, "[%s][%llu:%s:%s:%d]:", kLogLevelStr[level], nTid, pFileStart, func, line);
