...
public:
    /// The functor implementation should not contain other static object reference, for the initialize order is not certain
    explicit Initialize(std::function<bool(void)> f, std::function<void(void)> e) throw(InitializeRTEXCP) {
        if (!f()) {
