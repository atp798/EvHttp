...
EvHttpResp::EvHttpResp(struct evhttp_request *req) throw(EvHttpRespRTEXCP):evReq_(req) {
    if(nullptr == evReq_){
...
std::string EvHttpResp::GetHeadParam(std::string const &strKey){
    if(headParams_){
        ...
        if(val){ return std::string(val); } /// Must judge herer to prevent val be NULL
    }
    return std::string("");  
}
...
std::string EvHttpResp::GetPostParam(std::string const &strKey){
    /// Judge here is not useless, in most case, judge in the functional call is a waste
    if(!postParamParsed){
        parsePostParam();
    }
...
int EvHttpResp::GetUriPort(){
    return evhttp_uri_get_port(evUri_);
}
...
std::string EvHttpResp::GetUriQuery(){
    if(evUri_){
        const char * query = evhttp_uri_get_query(evUri_);
        if(query){ return std::string(query); }
    }
    return std::string("");
}
...
std::string EvHttpResp::GetUriFragment(){
    if(evUri_){
        ...
        if(fragment){ return std::string(fragment); }
    }
    return std::string("");
}
...
bool EvHttpResp::AddRespFile(std::string const &fileName){
    ...
    if(fd == -1){
        Utilis::LogError("Add file content to response body failed: open file error\n");
        return false;
    }
    Utilis::DEFER([&]{
        if (fd != -1) close(fd);
    ...
    if(evbuffer_add_file(respBuf_, fd, 0, len) == -1){
        Utilis::LogError("Add file content to response body failed: evbuffer_add_file error\n");
        return false;
    }
    return true;
}
